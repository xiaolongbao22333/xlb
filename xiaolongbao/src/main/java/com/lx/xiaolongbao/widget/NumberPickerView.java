...
public class NumberPickerView extends LinearLayout implements View.OnClickListener, TextWatcher, TextView.OnEditorActionListener, ViewTreeObserver.OnGlobalLayoutListener {
    ...
    private void initNumberPickerView(final Context context, AttributeSet attrs) {
        //加载定义好的布局文件
        View view = LayoutInflater.from(context).inflate(R.layout.number_button, this);
//        AutoUtils.auto(view);
        mRoot = (LinearLayout) findViewById(R.id.root);
        subText = (JImageView) findViewById(R.id.button_sub);
        addText = (JImageView) findViewById(R.id.button_add);
        ...
        mNumText.setOnEditorActionListener(this);
        //获取自定义属性的相关内容
...
//        Drawable dividerDrawable = typedArray.getDrawable(R.styleable.NumberButton_individer);
        ...
        //+和-文本的颜色
        int textColor = typedArray.getColor(R.styleable.NumberButton_numTextColor, 0xff000000);
        //+和-文本的字体大小
        int textSize = typedArray.getDimensionPixelSize(R.styleable.NumberButton_numTextSize, -1);
        // 中间显示数量的按钮宽度
...
//        }
        ...
        mNumText.setSelection(String.valueOf(getNumText()).length());
    }
    ...
    public NumberPickerView setCurrentNum(int currentNum) {
        if (currentNum > minDefaultNum) {
            if (currentNum <= currentInventory) {
                ...
                currentNumber = currentInventory;
    ...
    public void onClick(View view) {
        ...
        if (onNumberChangeListener != null) {
            onNumberChangeListener.onChange(Integer.parseInt(mNumText.getText().toString()));
        }
    }
    ...
    public void afterTextChanged(Editable editable) {
        ...
        try {
            ...
            if (String.valueOf(currentNumber).equals(inputText) || (currentNumber == 0 && inputText.isEmpty())) {
                mNumText.addTextChangedListener(this);
                mNumText.setSelection(mNumText.getText().toString().length());
                return;
            }
    ...
    public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
        if (actionId == EditorInfo.IME_ACTION_DONE) {   // 按下完成按钮，这里和上面imeOptions对应
            if (TextUtils.isEmpty(mNumText.getText().toString())) {
                mNumText.setText("1");
                currentInventory = 1;
            }
            mNumText.setCursorVisible(false);
            return false;
        }
    ...
    public void onGlobalLayout() {
        ...
        int screenHeight = mRoot.getRootView()
                .getHeight();
        int heightDifference = screenHeight - (r.bottom);
        if (heightDifference > 200) {
            //软键盘显示
            mNumText.setCursorVisible(true);
// changeKeyboardHeight(heightDifference);
    ...
    public void setViewEnabled(boolean isEnabled) {
        mRoot.setEnabled(isEnabled);
        mNumText.setEnabled(isEnabled);
        addText.setEnabled(isEnabled);
